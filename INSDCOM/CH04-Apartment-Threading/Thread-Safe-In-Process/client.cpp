...
void __stdcall MyThread(IStream* pScream)
{
	...
	for(int count = 0; count < 10; count++)
	{
		...
		printf("[WorkThread=%d] Client: Calling Sum(%d, %d) = %d\n", mytid, count, count, sum);
	}
...
{
	DWORD mytid = GetCurrentThreadId();
	cout << "Client: Calling CoInitialize(). Thread id of MainThread=" << mytid << endl;
	...
	if(SUCCEEDED(hr)) {
		...
		printf("[MainThread=%d] Client: Calling Sum(2, 3) = %d\n", mytid, sum);
	}
