...
public:
	...
	CMMFSparse(HANDLE hStream = NULL, 
		DWORD dwStreamSizeMaxLow = 0, DWORD dwStreamSizeMaxHigh = 0);
...
CMMFSparse::CMMFSparse(HANDLE hStream, 
	DWORD dwStreamSizeMaxLow, DWORD dwStreamSizeMaxHigh) 
{
	Initialize(hStream, dwStreamSizeMaxLow, dwStreamSizeMaxHigh);
}

BOOL CMMFSparse::Initialize(HANDLE hStream, 
	DWORD dwStreamSizeMaxLow, DWORD dwStreamSizeMaxHigh) 
{
	if (m_hFileMap != NULL) 
		ForceClose();
