...
void CD3DSettingsDialog::WindowedFullscreenChanged( void )
{
    ...
    if( IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        _sntprintf_s( strResolution, _TRUNCATE, TEXT("%d by %d"), m_d3dSettings.Windowed_DisplayMode.Width, 
            m_d3dSettings.Windowed_DisplayMode.Height );
        strResolution[49] = 0;
        ...
        if( m_d3dSettings.Windowed_DisplayMode.RefreshRate == 0 )
            ...
            _sntprintf_s( strRefreshRate, _TRUNCATE, TEXT("%d Hz"), m_d3dSettings.Windowed_DisplayMode.RefreshRate );
        strRefreshRate[49] = 0;
    ...
    if( !IsDlgButtonChecked( m_hDlg, IDC_WINDOW ) )
    {
        ...
        for( UINT idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
        {
            ...
            if (displayMode.Format == adapterFormat)
            {
                ...
                _sntprintf_s( strResolution, _TRUNCATE, TEXT("%d by %d"), displayMode.Width, displayMode.Height );
                strResolution[49] = 0;
    ...
    for( UINT idm = 0; idm < pAdapterInfo->pDisplayModeList->Count(); idm++ )
    {
            ...
            displayMode.Height == height)
        {
            ...
            if( displayMode.RefreshRate == 0 )
                ...
                _sntprintf_s( strRefreshRate, _TRUNCATE, TEXT("%d Hz"), displayMode.RefreshRate );
            strRefreshRate[49] = 0;
