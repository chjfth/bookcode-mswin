...
LONG WINAPI DelayLoadDllExceptionFilter(PEXCEPTION_POINTERS pep, int *py);
...
{
	__try {
		sprintf(g_buf, "%d", 456); //(A)
		return bobo2();
	}
	__finally {
		OutputDebugStringA("bobo1 __finally...\n"); //(B)
	}

	// memo: @ebp is the same when break at (A) or (B)
}
...
int WINAPI _tWinMain(HINSTANCE hinstExe, HINSTANCE, PTSTR pszCmdLine, int) {
   ...
   __try {
      ...
      x = bobo1();  // Attempt to call delay-load function
   ...
   __except (DelayLoadDllExceptionFilter(GetExceptionInformation(), &y)) {
...
LONG WINAPI DelayLoadDllExceptionFilter(PEXCEPTION_POINTERS pep, int *py) {
   ...
   case VcppException(ERROR_SEVERITY_ERROR, ERROR_MOD_NOT_FOUND): // 0xC06d007E
      // The DLL module was not found at runtime
      wsprintfA(sz, "Dll not found: %s", pdli->szDll); 
      break;

   case VcppException(ERROR_SEVERITY_ERROR, ERROR_PROC_NOT_FOUND): // C06d007F
