...
void dbgprint_excinfo(EXCEPTION_POINTERS *pep, TCHAR *funcname)
{
	EXCEPTION_RECORD *pEXR = pep->ExceptionRecord;
	DWORD exccode = pEXR->ExceptionCode;
	...
	if(exccode==EXCEPTION_ACCESS_VIOLATION)
	{
		...
		int offset = (CELL*)pvAddrTouched - g_cellbase;
		int row = offset/g_nNumCols;
		int col = offset%g_nNumCols;
		
		dbgprint(TEXT("In %s(), 0xC0000005 (attempt %s @ %p [%d,%d])"),
			funcname,
	...
	{
		dbgprint(TEXT("In CVMArray<TYPE>::UnhandledExceptionFilter(), exccode=0x%0X"), exccode);		
	}
}
   ...
   BOOL fRetryUntilSuccessful) 
{
   ...
   dbgprint_excinfo(pep, TEXT("CVMArray<TYPE>::ExceptionFilter")); // chj test
...
LONG WINAPI CVMArray<TYPE>::UnhandledExceptionFilter(PEXCEPTION_POINTERS pep) 
{
   ...
   LONG lDisposition = EXCEPTION_CONTINUE_SEARCH;
	...
	dbgprint_excinfo(pep, TEXT("CVMArray<TYPE>::UnhandledExceptionFilter")); // chj test
