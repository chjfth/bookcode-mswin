...
BOOL WINAPI EjectLibW(DWORD dwProcessId, PCWSTR pszLibFile) {
	...
	__try {
		...
		const TCHAR *dllfilename = _tcsrchr(pszLibFile, TEXT('\\'))+1; // +1 to skip the \
		...
		for (; fMoreMods; fMoreMods = Module32NextW(hthSnapshot, &me)) {
			founds++;
			dbgprint(L"EjectLibW found(%d): %s | %s", founds, me.szModule, me.szExePath);
			
			//fFound = (lstrcmpiW(me.szModule,  pszLibFile) == 0) || (lstrcmpiW(me.szExePath, pszLibFile) == 0);
				// Chj: This is buggy, because when pszLibFile has extra breadcrumb like:
				//	D:\w\personal\chj\bookcode\PrgAppWin-4th\22-InjLib\.\..\x64\Debug\22 ImgWalk.DLL
				// the lstrcmpiW fails.
			// So I check only DLL filename.
			fFound = 0==lstrcmpiW(dllfilename, me.szModule);
			if (fFound) 
				break;
		}
		if (!fFound) 
...
void Dlg_OnCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify) {
   ...
   switch (id) {
      ...
      case IDC_INJECT:
		 ...
		 dbgprint(_T("GetModuleFileName(NULL,,) returns: %s"), szLibFile);
