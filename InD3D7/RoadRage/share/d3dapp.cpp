...
VOID CD3DApplication::ShowStats()
{
    ...
    TCHAR buffer[80];
    _sntprintf_s( buffer, _TRUNCATE, _T("%7.02f fps (%dx%dx%d)"), fFPS,
...
VOID CD3DApplication::DisplayFrameworkError( HRESULT hr, DWORD dwType )
{
    ...
    switch( hr )
    {
        ...
        case D3DENUMERR_NOCOMPATIBLEDEVICES:
            lstrcpy( strMsg, _T("Could not find any compatible Direct3D devices.") );
            break;
        case D3DENUMERR_SUGGESTREFRAST:
            lstrcpy( strMsg, _T("Could not find any compatible devices.\n\n")
                     _T("Try enabling the reference rasterizer using\n")
                     _T("EnableRefRast.reg.") );
            break;
		...
		case D3DFWERR_NODIRECT3D: // Chap5
		...
		case D3DFWERR_NO3DDEVICE: // Chap5
        ...
        case D3DFWERR_NONZEROREFCOUNT:
            lstrcpy( strMsg, _T("A DDraw object has a non-zero reference count\n")
                     _T("(meaning it was not properly cleaned up).") );
            break;
