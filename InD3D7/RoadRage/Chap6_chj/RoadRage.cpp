...
VOID CMyD3DApplication::ShowStats()
{
	...
	_sntprintf_s( buffer, _TRUNCATE, _T("%7.02f fps (%dx%dx%d)"), fFPS, ddsd.dwWidth,
		ddsd.dwHeight, ddsd.ddpfPixelFormat.dwRGBBitCount );
	OutputText( 0, 0, buffer );

	_sntprintf_s( buffer, _TRUNCATE, _T("health : %d"), MyHealth);
...
void CMyD3DApplication::InitRRvariables()
{
	...
	for(i = 0; i < 10; i++)
	{
		...
		_tcscpy_s(player_list[i].name, _T(""));
	}
...
BOOL CMyD3DApplication::LoadRR_Resources()
{
	...
	OpenSoundFile(_T("colt.wav"), 0);
	OpenSoundFile(_T("engine.wav"), 1);
...
//	D3DMATRIX matWorld, matProj;
	D3DUtil_SetIdentityMatrix( matWorld );
	m_pd3dDevice->SetTransform( D3DTRANSFORMSTATE_WORLD,      &matWorld ); // todo: add m_
	...
	m_pd3dDevice->LightEnable( 0, TRUE ); // chap 5 (chj:can delete?)
	m_pd3dDevice->SetRenderState( D3DRENDERSTATE_LIGHTING, bEnableLighting ); // chap 6

	//m_pd3dDevice->SetRenderState( D3DRENDERSTATE_LIGHTING, TRUE ); // chap 5
	//m_pd3dDevice->SetRenderState( D3DRENDERSTATE_AMBIENT,  0xFF202020 ); // 0xAARRGGBB chap 5
...
BOOL CALLBACK FireWeapon(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)         
{   
...
INT WINAPI _tWinMain( HINSTANCE hInst, HINSTANCE, LPTSTR strCmdLine, INT )
{
	...
	{
		MessageBox( NULL, _T("Error initializing DirectSound.  Sample will now exit."), 
			_T("DirectSound Sample"), MB_OK | MB_ICONERROR );
		return FALSE;
	}
