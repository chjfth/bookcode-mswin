...
#ifndef __D3DAPP_hpp__
#define __D3DAPP_hpp__
...
#define D3DAPP_MAX_TEXTURE_FILENAME_LENGTH 1024
...
typedef struct tagD3DAppInfo {
    ...
    int			    CurrTextureFormat; /* current texture format will only change
										 if driver changes or when app changes it*/
    ...
    int						NumTextures;    /* number of textures in D3DApp's
											texture list */
    char					ImageFile[D3DAPP_MAXTEXTURES][D3DAPP_MAX_TEXTURE_FILENAME_LENGTH]; /* files */
    LPDIRECTDRAWSURFACE7    lpTextureSurf[D3DAPP_MAXTEXTURES]; /* surfaces */
     ...
     */
    LPDIRECTDRAW7			lpDD7;	   /* DirectDraw object */
	...
	DDPIXELFORMAT			ddpfZBuffer;      // Enumerated zbuffer format
    DDPIXELFORMAT			ddpfTextureFormats;
	DDPIXELFORMAT			ddpfBackBuffer;
...
typedef struct tagD3DAppRenderState {
	...
	BOOL			 bAlphaTransparency; /* bAlphaTransparency is on */
 ...
 */
BOOL D3DAppCreateSurface(DDSURFACEDESC2 *ddsd, LPDIRECTDRAWSURFACE7 *lplpSurf);
