 ...
 */
{
  ...
  if (cinfo->num_scans <= 0)
    cinfo->ERREXIT1(JERR_BAD_SCAN_SCRIPT, 0);
  ...
  for (scanno = 1; scanno <= cinfo->num_scans; scanptr++, scanno++) {
    ...
    if (ncomps <= 0 || ncomps > MAX_COMPS_IN_SCAN)
      cinfo->ERREXIT2(JERR_COMPONENT_COUNT, ncomps, MAX_COMPS_IN_SCAN);
    for (ci = 0; ci < ncomps; ci++) {
      thisi = scanptr->component_index[ci];
      if (thisi < 0 || thisi >= cinfo->num_components)
	cinfo->ERREXIT1(JERR_BAD_SCAN_SCRIPT, scanno);
...
#define MAX_AH_AL 13
	...
	cinfo->ERREXIT1(JERR_BAD_PROG_SCRIPT, scanno);
      if (Ss == 0) {
  ...
  if (cinfo->progressive_mode) {
	...
	cinfo->ERREXIT(JERR_MISSING_DATA);
    }
#endif
  } else {
    for (ci = 0; ci < cinfo->num_components; ci++) {
...
jinit_c_master_control (j_compress_ptr cinfo, boolean transcode_only)
{
  ...
  master = (my_master_ptr) cinfo->mem->alloc_small(JPOOL_IMAGE, sizeof(my_comp_master));
