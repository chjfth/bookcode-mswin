...
struct jpeg_common_struct 
{
	...
	jpeg_common_struct(void)
	{
		err = NULL;
		mem = NULL;
	}

	void jpeg_destroy(void);
	...
	*/
	...
	void ERREXIT1(int code, int p1);
	void ERREXIT2(int code, int p1, int p2);
	void ERREXIT3(int code, int p1, int p2, int p3);
	void ERREXIT4(int code, int p1, int p2, int p3, int p4);
	void ERREXITS(int code, const char * mess);

	void WARNMS(int code);
	void WARNMS1(int code, int p1);
	void WARNMS2(int code, int p1, int p2);
};
...
struct jpeg_compress_struct : public jpeg_common_struct
{
...
struct jpeg_decompress_struct : public jpeg_common_struct
{
  /* Source of compressed data */
  class jpeg_source_mgr * src;
  ...
  struct jpeg_color_quantizer * cquantize;
	...
	boolean jpeg_input_complete(void);
	int		jpeg_consume_input(void);
	int		jpeg_read_header (boolean require_image);
	void    default_decompress_parms(void);
	void	jpeg_destroy_decompress(void);
	void	jpeg_abort_decompress(void);
	void	jpeg_CreateDecompress (int version, size_t structsize);
	boolean jpeg_has_multiple_scans(void);
	boolean jpeg_finish_decompress (void);
	boolean jpeg_start_decompress(void);

	JDIMENSION jpeg_read_scanlines(JSAMPARRAY scanlines, JDIMENSION max_lines);

	void	jpeg_create_decompress(void)
	{
		jpeg_CreateDecompress(JPEG_LIB_VERSION, sizeof(jpeg_decompress_struct));
	}
...
public:
	...
	jpeg_error_mgr(void);
	virtual ~jpeg_error_mgr();

	virtual void error_exit(j_common_ptr cinfo);
  
	// Conditionally emit a trace or warning message
	virtual void emit_message(int msg_level);
