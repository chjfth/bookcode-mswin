...
void KDDBView::Test_LargestDDB(HDC hDC, const RECT * rcPaint)
{
	...
	for (y=0; y<ARRAYSIZE(desp); y++)
	{
		...
		DecodeDDB(hBmp[0], mess, ARRAYSIZE(mess));
...
void KDDBView::Test_LoadBitmap(HDC hDC, const RECT * rcPaint)
{
	...
	for (int i=0; i<6; i++)
	{
		...
		dib.DecodeDIBFormat(mess, ARRAYSIZE(mess));
		TextOut(hDC, x, y + info.bmHeight+20, mess, _tcslen(mess));
		
		DecodeDDB(bmp.m_hObj, mess, ARRAYSIZE(mess));
...
void KDIBSectionView::Test_CreateDIBSection(HDC hDC, const RECT * rcPaint)
{
	...
	for (int i=0; i<ARRAYSIZE(bitcount); i++)
	{
		...
		dibsection[i].DecodeDIBSectionFormat(temp, ARRAYSIZE(temp));
		...
		if ( dibsection[i].GetBitmap() )
		{
			...
			DecodeDDB(hBig, temp, ARRAYSIZE(temp));
