...
BOOL RemoveFont(const TCHAR * fontname, int option, HANDLE hFont)
{
	...
	_tcscpy_s(fotfile, MAX_PATH, ttffile);

	wsprintf(ttffile + _tcslen(ttffile), _T("\\%s.ttf"), fontname);
	wsprintf(fotfile + _tcslen(fotfile), _T("\\%s.fot"), fontname);
...
void KFileDialog::SetupOFN(OPENFILENAME & ofn, TCHAR Filter[], HWND hWnd, const TCHAR * pExtension, const TCHAR * pClass)
{
	...
	wsprintf(Filter, _T("%s (*.%s)%c*.%s%c"), pClass, pExtension, 0, pExtension, 0);
...
BOOL GenerateDoc(const TCHAR * docname)
{
	...
	while ( fi.GetOpenFileName(NULL, _T("ttf"), _T("TrueType fonts")) )
	{
		...
		int len = _tcslen(fi.m_TitleName);
...
class KMyDialog : public KDialog
{
	...
	void Update(void)
	{
		for (int i=0; FontList[i]; i++)
		{
			...
			_tcscpy_s(logfont.lfFaceName, ARRAYSIZE(logfont.lfFaceName), FontList[i]);
			...
			{
				_tcscpy_s(name, ARRAYSIZE(name), (TCHAR*)outm+(int)outm[0].otmpFamilyName);
				_tcscat_s(name, ARRAYSIZE(name), _T(" "));
				_tcscat_s(name, ARRAYSIZE(name), (TCHAR*)outm+(int)outm[0].otmpStyleName);
			}
	...
	virtual BOOL OnCommand(HWND hWnd, WPARAM wParam, LPARAM lParam)
	{
		switch ( LOWORD(wParam) )
		{
			...
			case IDC_GENERATE:
				GenerateDoc(_T("testdoc.tmp"));
				return TRUE;

			case IDC_LOAD:
			{
				GetOption();
				int succeeded = ProcessDocument(_T("testdoc.tmp"), m_option, true, hFont);

				TCHAR mess[32];
				wsprintf(mess, _T("%d font(s) installed."), succeeded);
				MessageBox(NULL, mess, _T("Font Install"), MB_OK);
			...
			{
				GetOption();
				int succeeded = ProcessDocument(_T("testdoc.tmp"), m_option, false, hFont);

				TCHAR mess[32];
				wsprintf(mess, _T("%d font(s) uninstalled."), succeeded);
				MessageBox(NULL, mess, _T("Font Install"), MB_OK);
			...
			case WM_FONTCHANGE:
				{
					...
					wsprintf(mess, _T("WM_FONTCHANGE %d"), ++m_nFontChange);
