/*----------------------------------------
   Record2.C -- MCI Command String Tester
	...
	case WM_COMMAND:
		...
		case IDC_RECORD_BEG:
			...
			dwError = mciSendCommand (0, MCI_OPEN, 
				...
				(DWORD_PTR)(LPMCI_OPEN_PARMS)&mciOpen) ;
			if (dwError != 0)
			{
			...
			mciSendCommand (wDeviceID, MCI_RECORD, MCI_NOTIFY,
				(DWORD_PTR)(LPMCI_RECORD_PARMS)&mciRecord) ;
		...
		case IDC_RECORD_END:
			...
			mciSendCommand (wDeviceID, MCI_SAVE, MCI_WAIT | MCI_SAVE_FILE,
				(DWORD_PTR)(LPMCI_SAVE_PARMS)&mciSave) ;
		...
		case IDC_PLAY_BEG:
			...
			mciSendCommand (wDeviceID, MCI_PLAY, MCI_NOTIFY,
				(DWORD_PTR)(LPMCI_PLAY_PARMS)&mciPlay) ;
