...
Chj: Add code to debug GetTextExtentPoint32() error accumulation behavior.
  -----------------------------------------*/
	...
	PSTR szCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow (szAppName, TEXT ("Justified Type #1 (Dbg GetTextExtentPoint32)"),
		WS_OVERLAPPEDWINDOW,
...
LRESULT CALLBACK WndProc (HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		// Chj: I'll use Arial 12pt as initial font.
		memset(&lf, 0, sizeof(lf));
		_tcscpy_s(lf.lfFaceName, ARRAYSIZE(lf.lfFaceName), _T("Arial"));
		HDC hdc = GetDC(NULL);
		lf.lfHeight = -12 * GetDeviceCaps(hdc, LOGPIXELSX)/72;
		ReleaseDC(NULL, hdc);
...
/*
			rect.right  = GetDeviceCaps (hdcPrn, PHYSICALWIDTH) -
				...
				GetDeviceCaps (hdcPrn, PHYSICALOFFSETX) ;
*/
			// Chj: Pass "same inch" width to printer DC, by setting correct rect.right .
			//
			HDC hdc = GetDC(hwnd); // first acquire screen DC to know "screen page inch"
			RECT rScreen = {};
			GetClientRect(hwnd, &rScreen);
			rScreen.left  += GetDeviceCaps (hdc, LOGPIXELSX) / 2 ;
			rScreen.right -= GetDeviceCaps (hdc, LOGPIXELSX) / 4 ;
			double dInchWidth = (double)(rScreen.right-rScreen.left)/GetDeviceCaps (hdc, LOGPIXELSX);
			
			rect.right = rect.left + (int)(dInchWidth*GetDeviceCaps (hdcPrn, LOGPIXELSX));
			...
			if ((StartDoc (hdcPrn, &di) > 0) && (StartPage (hdcPrn) > 0))
			{
				...
				FrameRect(hdcPrn, &rect, (HBRUSH)GetStockObject(BLACK_BRUSH));
